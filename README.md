# BananaPod｜图像生成与编辑工作台（v0.9.8）

轻量、可扩展的图像生成与编辑工作台，采用 PodUI 主题。统一第三方 API 为 OpenAI 风格接口（基于 `whatai.cc`），支持图层系统、裁剪与合并、双语界面、iPad/Apple Pencil 场景。

## 发布亮点（qd1.0.4 / v0.9.8）
- 清晰醒目的输入占位与快捷效果文案，聚焦“结构表达”与“概念呈现”。
- 双语提示词 1:1 映射，确保中英文行为一致（修复中文多行与引号）。
- 剖面/爆炸分析/方案线稿等提示词优化，强调“层次清晰、重点醒目”。
- 项目简介更简洁醒目，突出 Aura 主题与双语支持。

## 核心更新（v0.9.8）
- 输入占位与快捷效果文案统一为“清晰醒目”的描述方式，提升剖面、爆炸分析、方案线稿等结构表达类任务的稳定性与可读性。
- 双语提示词对齐：`QUICK_PROMPTS_EN` 与 `QUICK_PROMPTS_ZHO` 保持 1:1 映射，修复中文多行字符串/引号问题。
- 项目简介更新：`metadata.json` 描述更简洁，强调 Aura 主题、双语与“醒目呈现”。
