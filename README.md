...
> 当前代码对应 Git 标签：`qd1.0.6`（分支：`rollback-qd0.1.0`）

---

## 版本亮点（qd1.0.6）

- 多图场景性能明显优化：限制画布历史记录深度，避免长时间编辑导致的卡顿。
- 属性编辑体验升级：为不透明度、圆角、线宽、字号等高频属性引入「预览 / 提交」双通道。
- 图片导入稳定性提升：修复拖拽图片到画布时可能导入多张重复图片的问题。
- 构建产物校验：通过 `npm run build` 完成生产构建，生成 `dist/` 产物，可直接部署。
...
- 当前代码标签：`qd1.0.6`
- 近期关键变更：
  - 统一图片返回 `url`，兼容多种模型输出格式。
  - `nano-banana-2` 自动裁剪到最近枚举比例，`images/edits` 去除 `size` 参数。
  - 新增脚本 `npm run test:banana` 用于验证生图 / 编辑链路。
  - 引入历史深度限制与属性编辑预览/提交双通道，优化多图场景性能。
