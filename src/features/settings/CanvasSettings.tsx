...
const MODELS = [
    { id: 'gemini-2.5-flash-image', priceZH: '¥0.08/次', priceEN: '¥0.08/call' },
    { id: 'gemini-3-pro-image-preview', priceZH: '¥0.3/次', priceEN: '¥0.3/call' },
    { id: 'nano-banana', priceZH: '¥0.16/次', priceEN: '¥0.16/call' },
    { id: 'nano-banana-2', priceZH: '¥0.4/次', priceEN: '¥0.4/call' },
];
        ...
        >
            <Panel className="relative flex flex-col w-80 max-h-[85vh] overflow-hidden shadow-2xl">
                ...
                <div className="flex-grow overflow-y-auto p-4 space-y-4 custom-scrollbar">
                    ...
                    {/* Models */}
                    <div className="space-y-1.5">
                        <label className="text-xs font-medium text-[var(--text-heading)]">{language === 'ZH' ? '图像模型' : 'Image Model'}</label>
                            ...
                            {MODELS.map(model => (
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between px-3 py-2 rounded-md border text-left transition-colors
                                        ${imageModel === model.id 
                                            ? 'bg-[var(--bg-accent-subtle)] border-[var(--text-accent)] ring-1 ring-[var(--text-accent)]' 
                                            : 'bg-[var(--bg-input)] border-[var(--border-color)] hover:bg-[var(--bg-hover)]'
                                        }`}
                                >
                                    <span className={`text-xs font-medium truncate pr-2 ${imageModel === model.id ? 'text-[var(--text-accent)]' : 'text-[var(--text-primary)]'}`}>
                                        {model.id}
                                    </span>
                                    <span className="text-[10px] text-[var(--text-secondary)] whitespace-nowrap">
                                        {language === 'ZH' ? model.priceZH : model.priceEN}
                                    </span>
                                </button>
                            ))}
                        </div>
