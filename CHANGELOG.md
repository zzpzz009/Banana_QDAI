...
- docs(readme): 更新 README 为 v0.9.8，新增“清晰醒目”文案策略与双语提示词 1:1 对齐说明
- i18n(prompts): 输入占位统一为“清晰醒目”风格（`translations.ts:131, 133, 239-247, 261, 263`）
- i18n(prompts): 优化剖面/爆炸分析/方案线稿等结构表达类提示词，使“层次清晰、重点醒目”（`translations.ts:22, 30, 52, 54, 62`）
- meta(description): 更新 `metadata.json` 描述为更简洁醒目，强调 Aura 主题与双语（`metadata.json:2-4`）
- build: `npm run lint` 与 `npm run build` 通过，产物生成于 `dist/`

验证说明：
- 打开应用并切换中英文界面：提示输入框的占位文案应明显引导“清晰醒目”的描述；快捷效果中的剖面/爆炸分析/方案线稿文案更聚焦结构表达
- 运行 `npm run build` 应成功完成构建，且界面文案按上述变更显示
