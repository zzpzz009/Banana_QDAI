                ...
                <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <button
                            ...
                            className={`pod-card text-left flex flex-col justify-between h-full ${imageModel === 'gemini-2.5-flash-image' ? 'active' : ''}`}
                        >
                            <div className="text-[10px] text-[var(--text-muted)] h-3 flex items-end">{language === 'zho' ? '模型' : 'Model'}</div>
                            <div className="text-xs font-semibold break-all text-[var(--text-heading)] leading-snug my-1">gemini-2.5-flash-image</div>
                            <div className="mt-auto">
                                <div className="text-[10px] text-[var(--text-muted)]">{language === 'zho' ? '价格' : 'Price'}</div>
                                <div className="text-xs font-semibold text-[var(--text-primary)]">{language === 'zho' ? '¥0.08/次' : '¥0.08/call'}</div>
                            </div>
                        </button>
                        <button
                            onClick={() => handleSetModel('gemini-3-pro-image-preview')}
                            className={`pod-card text-left flex flex-col justify-between h-full ${imageModel === 'gemini-3-pro-image-preview' ? 'active' : ''}`}
                        >
                            <div className="text-[10px] text-[var(--text-muted)] h-3 flex items-end">{language === 'zho' ? '模型' : 'Model'}</div>
                            <div className="text-xs font-semibold break-all text-[var(--text-heading)] leading-snug my-1">gemini-3-pro-image-preview</div>
                            <div className="mt-auto">
                                <div className="text-[10px] text-[var(--text-muted)]">{language === 'zho' ? '价格' : 'Price'}</div>
                                <div className="text-xs font-semibold text-[var(--text-primary)]">{language === 'zho' ? '¥0.3/次' : '¥0.3/call'}</div>
                            </div>
                        </button>
                        <button
                            onClick={() => handleSetModel('nano-banana')}
                            className={`pod-card text-left flex flex-col justify-between h-full ${imageModel === 'nano-banana' ? 'active' : ''}`}
                        >
                            <div className="text-[10px] text-[var(--text-muted)] h-3 flex items-end">{language === 'zho' ? '模型' : 'Model'}</div>
                            <div className="text-xs font-semibold break-all text-[var(--text-heading)] leading-snug my-1">nano-banana</div>
                            <div className="mt-auto">
                                <div className="text-[10px] text-[var(--text-muted)]">{language === 'zho' ? '价格' : 'Price'}</div>
                                <div className="text-xs font-semibold text-[var(--text-primary)]">{language === 'zho' ? '¥0.16/次' : '¥0.16/call'}</div>
                            </div>
                        </button>
                        <button
                            onClick={() => handleSetModel('nano-banana-2')}
                            className={`pod-card text-left flex flex-col justify-between h-full ${imageModel === 'nano-banana-2' ? 'active' : ''}`}
                        >
                            <div className="text-[10px] text-[var(--text-muted)] h-3 flex items-end">{language === 'zho' ? '模型' : 'Model'}</div>
                            <div className="text-xs font-semibold break-all text-[var(--text-heading)] leading-snug my-1">nano-banana-2</div>
                            <div className="mt-auto">
                                <div className="text-[10px] text-[var(--text-muted)]">{language === 'zho' ? '价格' : 'Price'}</div>
                                <div className="text-xs font-semibold text-[var(--text-primary)]">{language === 'zho' ? '¥0.4/次' : '¥0.4/call'}</div>
                            </div>
                        </button>
                    </div>
                </div>
