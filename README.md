...
> 当前代码对应 Git 标签：`qd1.1.2`（分支：`rollback-qd0.1.0`）

---

## 版本亮点（qd1.1.2）

- 香蕉按钮展开面板简化：仅保留嵌入的 LeaderAI 网页与场景卡片，移除右侧提示词侧边栏，避免受第三方网页复制限制影响整体体验。
- PromptBar 交互优化：点击 PromptBar 区域（模式切换、输入框、生成按钮）不再自动关闭香蕉面板，只在点击面板外部时收起。
- LeaderAI 外站入口：支持点击香蕉面板顶部「LeaderAI Prompt Lab」标题，在新标签页打开完整实验室页面，便于直接使用网页原生功能。
- Electron 支持与资源更新：补充 `electron/main.cjs` 主进程入口与应用图标资源，为桌面版打包与分发做准备。
- 规划文档完善：更新 `.trae/scratchpad.md`，记录香蕉面板交互重构计划与任务状态，便于后续协作与回溯。
...
- 近期关键变更：
  ...
  - `qd1.1.1`：多图场景性能优化、属性编辑预览/提交双通道、图片拖拽导入稳定性提升，以及构建产物校验（`npm run build` 生成可部署的 `dist/`）。
